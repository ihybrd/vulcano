...
def supported(extensions, layers):
    ...
    vklogging.logger.print("Device can support the following extensions:")
    vklogging.logger.log_list(supportedExtensions)
    
    for extension in extensions:
        
        if extension in supportedExtensions:
            vklogging.logger.print(f"Extension \"{extension}\" is supported!")
        else:
            vklogging.logger.print(f"Extension \"{extension}\" is not supported!")
            return False

    #check layer support
    supportedLayers = [layer.layerName for layer in vkEnumerateInstanceLayerProperties()]

    vklogging.logger.print("Device can support the following layers:")
    vklogging.logger.log_list(supportedLayers)

    for layer in layers:
        if layer in supportedLayers:
            vklogging.logger.print(f"Layer \"{layer}\" is supported!")
        else:
            vklogging.logger.print(f"Layer \"{layer}\" is not supported!")
            return False

    return True

def make_instance(applicationName):
    vklogging.logger.print("Making an instance...")
    ...
    vklogging.logger.print("extensions to be requested:")
    ...
    try:
        ...
        vklogging.logger.print("Failed to create Instance!")
        return None